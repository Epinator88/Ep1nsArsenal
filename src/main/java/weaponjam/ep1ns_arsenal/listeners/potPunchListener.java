    ...
    public void onPotPunch(PlayerInteractEvent ev)
    {
        if(ev.getAction().equals(Action.LEFT_CLICK_AIR)) {
            ...
            for (int i = 0; i < dist; i++) {
                pot = ev.getPlayer().getWorld().getNearbyEntities(ev.getPlayer().getLocation()
                        .add(v.getX() * i, v.getY() * i, v.getZ() * i), 1, 1, 1)
                        .stream().filter(entity -> entity instanceof Projectile).map(entity -> (Projectile) entity)
